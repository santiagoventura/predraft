...
<div class="mb-6">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('leagues.scoring.index', $league) }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
            Cancel
        </a>
    </div>
</div>
...
<div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
    ...
    <form action="{{ route('leagues.scoring.preset', $league) }}" method="POST" class="flex gap-4 items-end">
        @csrf
        <div>
...
<form action="{{ route('leagues.scoring.update', $league) }}" method="POST" x-data="scoringForm()">
    @csrf
    @method('PUT')
                    ...
                    <div class="flex gap-2 items-center border-b pb-2">
                        <div class="flex-1">
                            <select x-model="category.stat_code" 
                                ...
                                @foreach($batterStats as $code => $name)
                    ...
                    <div class="flex gap-2 items-center border-b pb-2">
                        <div class="flex-1">
                            <select x-model="category.stat_code"
                                    ...
                                    class="border rounded px-2 py-1 w-full text-sm">
                                <option value="">Select Stat...</option>
                                @foreach($pitcherStats as $code => $name)
                                    <option value="{{ $code }}">{{ $code }} - {{ $name }}</option>
                                @endforeach
                            </select>
                            <input type="hidden" x-model="category.stat_name"
...
function scoringForm() {
    const batterCategoriesData = @json($league->batterScoringCategories->values());
    const pitcherCategoriesData = @json($league->pitcherScoringCategories->values());
    ...
    return {
        ...
        batterStats: @json($batterStats),
        pitcherStats: @json($pitcherStats),
