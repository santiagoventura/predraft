<?php $__env->startSection('title', $draft->name); ?>
...
<div x-data="draftBoard()" x-init="init()" data-completed-picks="<?php echo e($summary['completed_picks']); ?>">
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold"><?php echo e($draft->name); ?></h1>
                <p class="text-gray-600"><?php echo e($draft->league->name); ?></p>
            </div>
            <div class="flex items-center space-x-4">
                ...
                <?php if($draft->status === 'setup'): ?>
                    <form action="<?php echo e(route('drafts.start', $draft)); ?>" method="POST" class="inline">
                        <?php echo csrf_field(); ?>
        ...
        <div class="lg:col-span-1 bg-white rounded-lg shadow p-6">
            ...
            <dl class="space-y-2">
                <div>
                    ...
                    <dd class="text-2xl font-bold"><?php echo e($draft->current_round); ?> / <?php echo e($draft->total_rounds); ?></dd>
                </div>
                <div>
                    <dt class="text-gray-600 text-sm">Picks</dt>
                    <dd class="text-lg font-semibold"><?php echo e($summary['completed_picks']); ?> / <?php echo e($summary['total_picks']); ?></dd>
                </div>
                <div class="pt-2 border-t">
                    <div class="text-sm">
                        <span>Hitters: <?php echo e($summary['hitters_picked']); ?></span>
                    </div>
                    <div class="text-sm">
                        <span>Pitchers: <?php echo e($summary['pitchers_picked']); ?></span>
                    </div>
                </div>
            </dl>
        </div>

        <?php if($draft->isInProgress() && $currentPick): ?>
                ...
                <div class="flex justify-between items-start mb-2">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-600 mt-2">
                            <?php echo e($currentPick->team->name); ?> (Pick #<?php echo e($currentPick->overall_pick); ?>)
                        </p>
                    </div>
                    <?php if($summary['completed_picks'] > 0): ?>
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center gap-3">
                                ...
                                <select x-model="simulateStopRound" class="border rounded px-3 py-2 text-sm w-24">
                                    <?php for($r = $draft->current_round; $r <= $draft->total_rounds; $r++): ?>
                                        <option value="<?php echo e($r); ?>"><?php echo e($r); ?></option>
                                    <?php endfor; ?>
                                </select>
                                <span class="text-sm text-gray-500">(of <?php echo e($draft->total_rounds); ?>)</span>
                            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <?php $__currentLoopData = $draft->league->teams; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $team): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[150px]
                                <?php echo e($currentPick && $currentPick->team_id === $team->id ? 'bg-blue-100' : ''); ?>">
                                <div class="flex items-center space-x-2">
                                    <span><?php echo e($team->name); ?></span>
                                    <?php if($currentPick && $currentPick->team_id === $team->id): ?>
                                        <span class="inline-block w-2 h-2 bg-blue-600 rounded-full animate-pulse"></span>
                                    <?php endif; ?>
                                </div>
                                <div class="text-xs text-gray-400 font-normal">Slot <?php echo e($team->draft_slot); ?></div>
                            </th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <?php $__currentLoopData = $positionSlots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $positionSlot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 sticky left-0 bg-white z-10">
                                <?php echo e($positionSlot); ?>
                            ...
                            <?php $__currentLoopData = $draft->league->teams; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $team): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm
                                    <?php echo e($currentPick && $currentPick->team_id === $team->id ? 'bg-blue-50' : ''); ?>"
                                    id="roster-cell-<?php echo e($team->id); ?>-<?php echo e($positionSlot); ?>"
                                    data-team-id="<?php echo e($team->id); ?>"
                                    data-position="<?php echo e($positionSlot); ?>">
                                    <?php if($rosterEntry): ?>
                                        <div class="flex flex-col">
                                            <span class="font-semibold text-gray-900"><?php echo e($rosterEntry->player->name); ?></span>
                                            <span class="text-xs text-gray-500"><?php echo e($rosterEntry->player->mlb_team); ?> - <?php echo e($rosterEntry->player->positions); ?></span>
                                        </div>
                                    <?php else: ?>
    ...
    <div class="bg-white rounded-lg shadow p-6" id="recent-picks-container">
        ...
        <div class="space-y-2" id="recent-picks-list">
            <?php $__empty_1 = true; $__currentLoopData = $recentPicks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pick): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="flex justify-between items-center p-3 bg-gray-50 rounded pick-item" data-pick-id="<?php echo e($pick->overall_pick); ?>">
                    <div>
                        <span class="font-semibold">Pick #<?php echo e($pick->overall_pick); ?>:</span>
                        <span class="ml-2"><?php echo e($pick->player->name); ?></span>
                        <span class="text-gray-600 ml-2">(<?php echo e($pick->player->positions); ?>)</span>
                    </div>
                    <div class="text-sm text-gray-600">
                        <?php echo e($pick->team->name); ?>
...
const availablePlayersData = <?php echo json_encode($availablePlayersJson, 15, 512) ?>;
...
function draftBoard() {
    return {
        ...
        selectedPlayerId: '',
        autoRefresh: true,
        lastPickCount: <?php echo e($summary['completed_picks']); ?>,
        // Simulation state
        simulating: false,
        simulateStopRound: <?php echo e($draft->current_round); ?>,
        ...
        async checkForUpdates() {
            try {
                const response = await fetch('<?php echo e(route('drafts.show', $draft)); ?>', {
                    headers: {
        ...
        async loadRecommendations() {
            this.loading = true;
            try {
                const response = await fetch('<?php echo e(route('drafts.recommendations', $draft)); ?>');
        ...
        async selectPlayer(rec) {
            ...
            try {
                const response = await fetch('<?php echo e(route('drafts.pick', $draft)); ?>', {
                    method: 'POST',
                    headers: {
        ...
        async revertLastPick() {
            ...
            try {
                const response = await fetch('<?php echo e(route('drafts.revert', $draft)); ?>', {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest',
        ...
        async startSimulation() {
            ...
            const numTeams = <?php echo e($draft->league->teams()->count()); ?>;
            const currentRound = <?php echo e($draft->current_round); ?>;
            const currentPick = <?php echo e($draft->current_pick); ?>;
            ...
            try {
                ...
                while (true) {
                    const response = await fetch('<?php echo e(route('drafts.simulate', $draft)); ?>', {
                        method: 'POST',
                        headers: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/drafts/show.blade.php ENDPATH**/ ?>